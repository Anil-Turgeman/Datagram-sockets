...
int main(int argc, char *argv[]){
  ...
  memset((char *) &c, 0, sizeof(c));	//Initializing the string to 0
  ...
  while(1) {
    ...
    if(from.sin_port == 3333){              //i got from a
      ...
      if(random > x){                       //if random bigger than X send the msg to C
		random = (double)rand() / (double)(RAND_MAX);
		printf("from A: \n\"%s\"\n", msg);
		fsize = sizeof(c);
		sendto(socket_fd, &msg, 100, 0, (struct sockaddr *)&c, fsize);
		printf("\nI've decided to send the msg to C\n");
		fflush(stdout);
			}else{
				printf("I've decided to drop it down\n");
			}
      }else{
        printf("From A: \n%s\n", msg);       //i got from c
        fsize = sizeof(a);
        sendto(socket_fd, &msg, 100, 0, (struct sockaddr *)&a, fsize);
        printf("I've sent the msg to A\n");
        fflush(stdout);
      }
    }
